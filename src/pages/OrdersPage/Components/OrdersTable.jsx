...
const OrdersTable = ({ numberOfItems, changeOrdersNumber }) => {
  ...
  useEffect(() => {
    ...
    const itemsCount = Math.floor((window.innerHeight * 0.9) / rowHeight) - 1;
  ...
  return (
    ...
    >
      {!loading ? (
        <table
          className={`min-w-[900px] sm:min-w-full border-separate border-spacing-0 ${!maxHeight ? "invisible" : "visible"}`}
        >
          <thead className="sticky top-0 z-10 bg-bg-main text-text-primary/85">
            <tr ref={th} className="bg-secondary/13 dark:bg-secondary/20">
              <th className="uppercase py-4 px-6 text-start text-xs">order</th>
              <th className="uppercase py-4 px-6 text-center text-xs">
                Customer
              </th>
              <th className="uppercase py-4 px-6 text-center text-xs">Date</th>
              <th className="uppercase py-4 px-6 text-center text-xs">
                Status
              </th>
              <th className="uppercase py-4 px-6 text-center text-xs">
              ...
              <th className="uppercase py-4 px-6 text-center text-xs">Total</th>
            </tr>
          </thead>
            ...
            {orders?.length > 0 ? (
              orders?.map((order, index, array) => (
                ...
                >
                  ...
                  >
                    <div className="grid grid-cols-[1fr_1.7fr] gap-4 items-center">
                      ...
                      <div className="min-w-0">
                        <h3 className="truncate text-left text-[14px] font-semibold uppercase text-text-primary/95">
                          {order.user?.username || "ــــــــــــ"}
                        </h3>

                        <p className="truncate text-left text-[12px] text-secondary/75">
                  ...
                  <td
                    className={`py-4 px-6 text-center text-xs text-secondary/90 ${
                      index === array.length - 1 ? "" : "border-b"
                    } border-secondary/15 dark:border-secondary/20`}
                  >
                  ...
                  >
                    <div className="flex flex-col items-center gap-1">
                      ...
                      <span className="text-xs text-secondary/90 capitalize">
                        {order.paymentMethod}
                      </span>
                    </div>
                  </td>
                  ...
                  <td
                    className={`py-4 px-6 text-center text-sm font-bold text-text-primary/80 ${
                      index === array.length - 1 ? "" : "border-b"
                    } border-secondary/15 dark:border-secondary/20`}
                  >
